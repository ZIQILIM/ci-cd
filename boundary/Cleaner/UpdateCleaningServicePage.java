...
public class UpdateCleaningServicePage extends JFrame {
    ...
    public UpdateCleaningServicePage(UserAccount cleanerAccount, int serviceId) {
        this.cleanerAccount = cleanerAccount;
        this.controller = new CleaningServiceController();
        this.serviceDAO = new CleaningServiceDAO();
        
        this.service = controller.findServiceById(serviceId);
    ...
    private void initializeUI() {
        setTitle("Update Cleaning Service");
        setSize(700, 750); // Increased height for categories
        ...
        JLabel headerLabel = new JLabel("Update Cleaning Service");
    ...
    private JPanel createButtonPanel() {
        ...
        cancelButton.addActionListener(e -> goBackButtonOnAction(e));
    ...
    private void loadCategories() {
        ...
        List<Integer> currentCategoryIds = serviceDAO.getCategoryIdsForService(service.getServiceId());
        ...
        if (availableCategories.isEmpty()) {
            ...
            for (ServiceCategory category : availableCategories) {
                ...
                // Check the box if this category is already assigned to the service
                if (currentCategoryIds.contains(category.getCategoryId())) {
    ...
    private void populateFields() {
        serviceNameField.setText(service.getTitle());
        descriptionArea.setText(service.getDescription());
        priceField.setText(String.valueOf(service.getPrice()));
        durationSpinner.setValue(service.getServiceDuration());
        
        String[] availableDays = service.getAvailableDays().split(",");
        for (String day : availableDays) {
            for (int i = 0; i < DAYS_OF_WEEK.length; i++) {
                if (DAYS_OF_WEEK[i].equals(day.trim())) {
        ...
        LocalTime startTime = service.getAvailableStartTime();
        LocalTime endTime = service.getAvailableEndTime();
        
        startTimeSpinner.setValue(java.util.Date.from(startTime.atDate(java.time.LocalDate.now())
            .atZone(java.time.ZoneId.systemDefault()).toInstant()));
        
        endTimeSpinner.setValue(java.util.Date.from(endTime.atDate(java.time.LocalDate.now())
            .atZone(java.time.ZoneId.systemDefault()).toInstant()));
    }
    ...
    private void updateService() {
        ...
        service.setAvailableDays(availableDays.toString());
        ...
        service.setCategoryIds(selectedCategoryIds);
        
        boolean success = controller.updateService(service.getServiceId(), service);
    ...
    public void displayUpdateServicePage() {
        setVisible(true);
    }
}