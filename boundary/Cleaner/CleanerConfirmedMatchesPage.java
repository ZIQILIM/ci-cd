...
import controller.Cleaner.ConfirmedMatchesController;
import entity.Homeowner.BookingService;
import entity.UserAdmin.UserAccount;
import utility.DateLabelFormatter;

public class CleanerConfirmedMatchesPage extends JFrame {
    private ConfirmedMatchesController controller;
    ...
    public CleanerConfirmedMatchesPage(UserAccount cleanerAccount) {
        this.cleanerAccount = cleanerAccount;
        this.controller = new ConfirmedMatchesController();
        initializeUI();
        ...
        fromCalendar.add(java.util.Calendar.MONTH, -1); 
    ...
    private JPanel createFilterPanel() {
        ...
        searchButton.addActionListener(e -> showConfirmedMatches());
        gbc.gridx = 1;
        gbc.gridy = 2;
        gbc.gridwidth = 2;
    ...
    private void loadServices() {
        List<Integer> serviceIds = controller.getCleanerServiceIds(cleanerAccount.getUsername());
        
        for (Integer serviceId : serviceIds) {
            String serviceName = controller.getServiceName(serviceId);
            serviceComboBox.addItem(new ServiceItem(serviceId, serviceName));
        }
    }
    
    private void showConfirmedMatches() {
        ...
        try {
            // Get confirmed matches using controller
            List<BookingService> matches = controller.getConfirmedMatches(
    ...
    // Display the confirmed matches page
